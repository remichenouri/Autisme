...
def show_aq10_and_prediction():
    """
    Fonction combinée pour l'évaluation AQ-10 et la prédiction TSA avec améliorations visuelles
    """
    import pandas as pd
    import plotly.express as px
    import plotly.graph_objects as go
    ...
    st.markdown(
        f"""<div style="background: linear-gradient(90deg, #3498db, #2ecc71); 
    ...
    <div style="background: linear-gradient(135deg, #e8f4fd 0%, #c3e8fb 100%); 
        ...
        <h3 style="color: #2c3e50; text-align: center; margin-top: 0; font-size: 1.8rem;">
            ℹ️ À propos de cette évaluation
        </h3>
        <p style="color: #2c3e50; line-height: 1.6;">
            ...
            entraîné sur plus de <strong>5 000 cas cliniques internationaux</strong>. Les données demandées correspondent 
            exactement aux variables utilisées lors de l'entraînement du modèle Random Forest.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.form("questionnaire_aq10_prediction", clear_on_submit=False):
        ...
        with col1:
            ...
            ethnies = ["Moyen-Orient", "Européen", "Asiatique", "Noir", "Hispanique", "Autre", "Latino"]
                    ...
                    try:
                        ...
                        st.markdown(f"""
                            <div class="result-card {color_class}">
                                ...
                                <div style="font-size: 1.8rem; font-weight: bold; color: #2c3e50; margin-bottom: 10px;">
                                    Résultat de la Prédiction IA
                                </div>
                                <div style="font-size: 2.5rem; font-weight: bold; color: #3498db; margin: 20px 0;">
                                    {probability_percentage}%
                                </div>
                                
                                <div style="background: rgba(52, 152, 219, 0.1); border-radius: 10px; padding: 20px; margin: 20px 0;">
                                    <h4 style="color: #2c3e50; margin-top: 0;">🎯 Seuil de Sensibilité à 30%</h4>
                                    <p style="color: #34495e; margin: 0;">
                                        Ce seuil optimisé permet de ne manquer aucun cas potentiel tout en maintenant 
                                        une spécificité acceptable (sensibilité > 95% dans nos tests).
                                    </p>
                                </div>

                                <div style="margin-top: 25px;">
                                    <p style="font-size: 1.1rem; color: #7f8c8d;">
                                        Classification : <strong>{prediction_class}</strong><br>
                                        {recommendation}
                                    </p>
                                </div>
                            </div>
                        ...
                        if antecedents == "Oui":
                            base_risk *= 2.2
                        risk_factor = min(10.0, base_risk)
                        ...
                        # Suite du code avec les autres visualisations...
                        # (Conserver les parties existantes du code pour les graphiques et recommandations)
